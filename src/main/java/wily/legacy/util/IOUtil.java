...
public class IOUtil {
    ...
    public static <K, V> Codec<Map<K, V>> createIdMapCodec(Codec<K> keyCodec, Codec<V> codec, String keyField) {
        return new Codec<>() {
            ...
            public <T> DataResult<T> encode(Map<K, V> input, DynamicOps<T> ops, T prefix) {
                return DataResult.success(ops.createMap(input.entrySet().stream().map(e -> Pair.of(keyCodec.encodeStart(ops, e.getKey()).resultOrPartial(Legacy4J.LOGGER::error).get(), codec.encodeStart(ops, e.getValue()).resultOrPartial(Legacy4J.LOGGER::error).get()))));
            }
        };
    }
    ...
    public record FallbackCodec<T>(Encoder<T> encoder, Decoder<T> decoder, Codec<T> fallback) implements Codec<T> {
        ...
        public <T1> DataResult<Pair<T, T1>> decode(DynamicOps<T1> dynamicOps, T1 t1) {
            ...
            return decoded.result().isEmpty() ? fallback.decode(dynamicOps, t1) : decoded;
        }

        @Override
        public <T1> DataResult<T1> encode(T t, DynamicOps<T1> dynamicOps, T1 t1) {
            DataResult<T1> encoded = encoder.encode(t, dynamicOps, t1);
            return encoded.result().isEmpty() ? fallback.encode(t, dynamicOps, t1) : encoded;
        }
    }
}
