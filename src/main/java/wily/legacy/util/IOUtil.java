...
public class IOUtil {
    ...
    public static <T> Predicate<T> registryMatches(Registry<T> registry, JsonObject o) {
        String name = registry.key()./*? if <1.21.11 {*//*location*//*?} else {*/identifier/*?}*/().getPath();
            ...
            a.forEach(ie -> {
                if (ie instanceof JsonPrimitive p && p.isString()) {
                    ...
                    if (s.startsWith("#"))
                        ...
                        /*? if <1.21.11 {*//*ResourceLocation*//*?} else {*/Identifier/*?}*/ l = FactoryAPI.createLocation(s.replaceFirst("!", ""));
