...
public class LegacySoundUtil {
    public static void playSimpleUISound(SoundEvent sound, float volume, float pitch, boolean randomPitch){
        ...
        Minecraft.getInstance().getSoundManager().play(new SimpleSoundInstance(sound./*? if <1.21.2 {*/getLocation/*?} else {*//*location*//*?}*/(), SoundSource.MASTER, volume,pitch + (randomPitch ? (source.nextFloat() - 0.5f) / 10 : 0), source, false, 0, SoundInstance.Attenuation.NONE, 0.0, 0.0, 0.0, true));
    }
