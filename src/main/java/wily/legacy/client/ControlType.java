...
public record ControlType(ResourceLocation id, Optional<Component> name, boolean isKbm,
    ...
    public ControlType(ResourceLocation id) {
        this(id, Optional.of(Component.translatable("legacy.options.controlType." + id.getPath())), false, Optional.of(new SizeableAsset<>(new ControlFont(id))), Optional.of(id.withPath("textures/gui/title/minecraft/%s.png".formatted(id.getPath()))));
    }
    ...
    public record ControlFont(ResourceLocation font, int iconHeight) {
        public static final Codec<ControlFont> CODEC = IOUtil.createFallbackCodec(RecordCodecBuilder.create(i -> i.group(ResourceLocation.CODEC.fieldOf("id").forGetter(ControlFont::font), Codec.INT.fieldOf("iconHeight").forGetter(ControlFont::iconHeight)).apply(i, ControlFont::new)), ResourceLocation.CODEC.xmap(ControlFont::new, ControlFont::font));

        public ControlFont(ResourceLocation font) {
            this(font, 15);
        }
    }
