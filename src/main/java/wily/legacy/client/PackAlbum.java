    ...
    public static class Selector extends AbstractWidget implements ActionHolder {
        ...
        public void renderTooltipBox(GuiGraphics graphics, int x, int y, int width, int height) {
            ...
            if (getSelectedAlbum() != null) {
                ...
                (sd ? Panel.sdLabelsCache : Panel.labelsCache).apply(getSelectedAlbum().displayName(), nameWidth)
                        //? if >=1.21.11 {
                        .visitLines(TextAlignment.LEFT, x + (sd ? 40 : 43), y + 8, lineHeight, graphics.textRenderer());
                        //?} else {
                        /*.render(graphics, MultiLineLabel.Align.LEFT, x + (sd ? 40 : 43), y + 8, lineHeight, true, 0xFFFFFFFF);
                        *///?}
                graphics.disableScissor();
                Identifier background = getSelectedAlbum().backgroundSprite.orElse(p ? getPackBackground(packRepository.getPack(getSelectedAlbum().getDisplayPackId())) : null);
                ...
                int left = x + (sd ? 5 : 8);
                scrollableRenderer.render(graphics, left, y + 40, descriptionWidth, visibleLines * lineHeight, () -> label
                        //? if >=1.21.11 {
                        .visitLines(TextAlignment.LEFT, left, y + 40, lineHeight, graphics.textRenderer())
                        //?} else {
                        /*.render(graphics, MultiLineLabel.Align.LEFT, left, y + 40, lineHeight, true, 0xFFFFFFFF)
                         *///?}
                );
                if (background != null) {
