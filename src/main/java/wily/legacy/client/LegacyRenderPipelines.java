...
public class LegacyRenderPipelines {
    public static final RenderPipeline LIGHTMAP = RenderPipelinesAccessor.register(RenderPipeline.builder().withLocation(Legacy4J.createModLocation("pipeline/lightmap")).withVertexShader("core/blit_screen").withFragmentShader("core/lightmap").withUniform("AmbientLightFactor",UniformType.FLOAT).withUniform("SkyFactor",UniformType.FLOAT).withUniform("BlockFactor",UniformType.FLOAT).withUniform("UseBrightLightmap",UniformType.INT).withUniform("BlockLightColor",UniformType.VEC3).withUniform("SkyLightColor",UniformType.VEC3).withUniform("NightVisionFactor",UniformType.FLOAT).withUniform("UnderwaterVisionFactor",UniformType.FLOAT).withUniform("DarknessScale",UniformType.FLOAT).withUniform("DarkenWorldFactor",UniformType.FLOAT).withUniform("BrightnessFactor",UniformType.FLOAT).withVertexFormat(DefaultVertexFormat.POSITION, VertexFormat.Mode.QUADS).withDepthWrite(false).withDepthTestFunction(DepthTestFunction.NO_DEPTH_TEST).build());
