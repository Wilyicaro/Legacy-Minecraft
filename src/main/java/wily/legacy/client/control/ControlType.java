...
public record ControlType(Identifier id, Optional<Component> name, boolean isKbm,
    ...
    public ControlType(Identifier id) {
    ...
    public static ControlType getActiveType() {
        ControlType type = !LegacyControlsOptions.lockControlTypeChange.get() && Legacy4JClient.controllerManager.isControllerTheLastInput() ||
    ...
    public static ControlType get(Identifier id) {
        return Legacy4JClient.controlTypesManager.map().get(id);
    }

    public static ControlType getOrEmpty(Identifier id) {
