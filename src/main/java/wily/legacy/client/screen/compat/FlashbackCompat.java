...
public final class FlashbackCompat {
    ...
    private static synchronized boolean initialize() {
        ...
        try {
            ...
            recordingButtonConstructor = flashbackButtonClass.getConstructor(int.class, int.class, int.class, int.class, Component.class, Button.OnPress.class, Identifier.class);
    ...
    public static AbstractWidget createReplayButton(Screen parent, AbstractWidget anchor) {
        ...
        try {
            ...
            return (AbstractWidget) replayButtonConstructor.newInstance(anchor.getRight() + 4, anchor.getY(), size, size, Component.translatable("flashback.open_replays"), (Button.OnPress) button -> openReplays(parent));
        } catch (ReflectiveOperationException exception) {
    ...
    private static int nextY(List<Integer> positions, int index) {
        ...
        return positions.isEmpty() ? Minecraft.getInstance().getWindow().getGuiScaledHeight() / 2 : positions.getLast() + 24 * (index - positions.size() + 1);
    }

    private static AbstractWidget createRecordingButton(int x, int y, String translationKey, String icon, Button.OnPress action) {
        try {
            Object button = recordingButtonConstructor.newInstance(x, y, 20, 20, Component.translatable(translationKey), action, Identifier.fromNamespaceAndPath("flashback", icon));
    ...
    private static void openReplays(Screen parent) {
        try {
            ...
            List<?> incompatibleMods = minecraft.hasShiftDown() ? List.of() : (List<?>) getReplayIncompatibleModsMethod.invoke(null);
