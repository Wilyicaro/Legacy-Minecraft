...
public class HelpAndOptionsScreen extends RenderableVListScreen {
    ...
    public static List<AbstractWidget> createPlayerSkinWidgets(){
        ...
        list.add(LegacyConfigWidget.createWidget(LegacyOptions.of(Minecraft.getInstance().options.mainHand())));
        return list;
    }
    ...
    public static final OptionsScreen.Section HOW_TO_PLAY = new OptionsScreen.Section(Component.translatable("legacy.menu.how_to_play"), s->Panel.centered(s, LegacySprites.PANEL, 240, Math.min(8, HowToPlayScreen.Section.list.size())*24+16), new ArrayList<>(List.of(o-> HowToPlayScreen.Section.list.forEach(s-> o.getRenderableVList().addRenderable(s.createButtonBuilder(o).build())))), ArbitrarySupplier.empty(), ((screen, section) -> new OptionsScreen(screen, section){
        @Override
        public void renderableVListInit() {
            getRenderableVList().init(panel.x + (panel.width - 225) / 2,panel.y + 8,225,panel.height-16);
        }
    }));
    ...
    public HelpAndOptionsScreen(Screen parent) {
        ...
        renderableVList.addRenderable(openScreenButton(Component.translatable("controls.title"),()-> new RenderableVListScreen(this,Component.translatable("controls.title"),r->r.addRenderables(Button.builder(Component.translatable("options.mouse_settings.title"), button -> this.minecraft.setScreen(new OptionsScreen(r.getScreen(), s->Panel.centered(s, 250,102),Component.translatable("options.mouse_settings.title"), LegacyOptions.of(minecraft.options.invertYMouse()), LegacyOptions.of(Minecraft.getInstance().options.sensitivity()), LegacyOptions.of(minecraft.options.mouseWheelSensitivity()), LegacyOptions.of(minecraft.options.discreteMouseScroll()), LegacyOptions.of(minecraft.options.touchscreen())))).build(),Button.builder(Component.translatable("controls.keybinds.title"), button -> this.minecraft.setScreen(new LegacyKeyBindsScreen(r.getScreen(),minecraft.options))).build(),Button.builder(Component.translatable("legacy.options.selectedController"), button -> this.minecraft.setScreen(new ControllerMappingScreen(r.getScreen(),minecraft.options))).build()))).build());
