...
public class LegacyIconHolder extends SimpleLayoutRenderable implements GuiEventListener, NarratableEntry, ControlTooltip.ActionHolder {
    ...
    public LegacyIconHolder itemHolder(int x, int y, int width, int height, ItemStack itemIcon, boolean isWarning, Identifier iconSprite, Vec2 offset, ArbitrarySupplier<Identifier> override) {
        setPos(x, y);
        setBounds(width, height);
    ...
    public static boolean usesSlotPadding(ItemStack item) {
        return !item.has(DataComponents.TOOL) && !item.is(Items.BOW) && !item.is(Items.FISHING_ROD);
    }
