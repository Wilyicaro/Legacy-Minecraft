...
public abstract class ListButton extends AbstractButton {
    ...
    protected void renderWidget(GuiGraphics guiGraphics, int mouseX, int mouseY, float delta) {
        ...
        ResourceLocation sprite = focused
                ? LegacyRenderUtil.getSpriteOrFallback(LegacySprites.LIST_BUTTON_HIGHLIGHTED, LegacySprites.BUTTON_HIGHLIGHTED)
                : LegacyRenderUtil.getSpriteOrFallback(LegacySprites.LIST_BUTTON, LegacySprites.BUTTON);
        FactoryGuiGraphics.of(guiGraphics).blitSprite(sprite, getX(), getY(), getWidth(), getHeight());
        FactoryGuiGraphics.of(guiGraphics).setColor(1.0f, 1.0f, 1.0f, 1.0f);
        renderScrollingString(guiGraphics, minecraft.font, 2, LegacyRenderUtil.getDefaultTextColor(!focused));
    }
    ...
    protected void renderScrollingString(GuiGraphics guiGraphics, Font font, int i, int color) {
        ...
        LegacyFontUtil.applySDFont(ignored -> LegacyRenderUtil.renderScrollingString(guiGraphics, font, getMessage(), textX, textY, Math.max(textX + 1, textRight), Math.max(textY + 1, textBottom), color, true));
    }
