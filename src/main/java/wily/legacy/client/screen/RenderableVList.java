...
public class RenderableVList {
            ...
            if (listHeight > 0) {
                ...
                if (i == InputConstants.KEY_PAGEDOWN) {
                    if (isDirectionFocused(ScreenDirection.DOWN))
                        while (accessor.getChildren().contains(lastFocus) && canScrollDown) mouseScrolled(true);
                    setLastFocusInDirection(ScreenDirection.DOWN);
                    return true;
                }
                if (i == InputConstants.KEY_PAGEUP) {
                    if (isDirectionFocused(ScreenDirection.UP))
                        while (accessor.getChildren().contains(lastFocus) && scrolledList.get() > 0)
                            mouseScrolled(false);
    ...
    public GuiEventListener getNextHorizontalRenderable(boolean left, boolean right) {
        if (right && isDirectionFocused(ScreenDirection.RIGHT) || left && isDirectionFocused(ScreenDirection.LEFT)) {
            ...
            if (left && isDirectionFocused(ScreenDirection.UP) || right && isDirectionFocused(ScreenDirection.DOWN))
                keyPressed(left ? InputConstants.KEY_UP : InputConstants.KEY_DOWN, false);
