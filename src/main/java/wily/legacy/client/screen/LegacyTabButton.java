...
public class LegacyTabButton extends AbstractButton {
    ...
    public static final Identifier DEFAULT_ICON_TYPE_ID = FactoryAPI.createVanillaLocation("sprite");
    public static final IconType<Identifier> DEFAULT_ICON_TYPE = new IconType<>(Identifier.CODEC, LegacyTabButton::iconOf);
    public static final Codec<IconHolder<Identifier>> DEFAULT_ICON_CODEC = Identifier.CODEC.xmap(r -> new IconHolder<>(DEFAULT_ICON_TYPE_ID, DEFAULT_ICON_TYPE, r), IconHolder::content);
    ...
    public static Render iconOf(ModsScreen.SizedLocation sized) {
        return (t, guiGraphics, i, j, f) -> FactoryGuiGraphics.of(guiGraphics).blit(sized.identifier(), t.getX() + t.getWidth() / 2 - 12, t.getY() + t.getHeight() / 2 - 12, 0, 0, 24, sized.getScaledHeight(24), 24, sized.getScaledHeight(24));
    }
    ...
    public record TabSprite(Identifier sprite) implements SpriteRender {
        public static final Codec<TabSprite> CODEC = Identifier.CODEC.xmap(TabSprite::new, TabSprite::sprite);

        @Override
        public Identifier getSprite(LegacyTabButton button) {
            return sprite();
        }
    }
    ...
    public record TabSprites(Identifier left, Identifier middle,
                             Identifier right) implements SpriteRender {
        public static final Codec<TabSprites> CODEC = RecordCodecBuilder.create(i -> i.group(Identifier.CODEC.fieldOf("left").forGetter(TabSprites::left), Identifier.CODEC.fieldOf("middle").forGetter(TabSprites::middle), Identifier.CODEC.fieldOf("right").forGetter(TabSprites::right)).apply(i, TabSprites::new));

        public Identifier byType(Type type) {
            return switch (type) {
                case LEFT -> left;
                case MIDDLE -> middle;
                case RIGHT -> right;
    ...
    public record IconType<T>(Codec<T> contentCodec, Function<T, Render> createIcon) {
        public DataResult<IconHolder<T>> parse(Identifier typeId, Dynamic<?> dynamic) {
    ...
    public static Codec<IconHolder<?>> createIconHolderCodec(String typeField, String valueField) {
        Codec<Identifier> typeCodec = Identifier.CODEC.fieldOf(typeField).codec();
        return new Codec<>() {
            @Override
            public <T> DataResult<Pair<IconHolder<?>, T>> decode(DynamicOps<T> ops, T input) {
                Dynamic<T> dynamic = new Dynamic<>(ops, input);
                DataResult<Identifier> idResult = typeCodec.parse(dynamic);
    ...
    public static final Codec<IconHolder<?>> ICON_HOLDER_CODEC = IOUtil.createFallbackCodec(createIconHolderCodec("type", "value"), DEFAULT_ICON_CODEC.xmap(Function.identity(), h -> (IconHolder<Identifier>) h));

    public record IconHolder<T>(Identifier typeId, IconType<T> type, T content, Render icon) {
        public IconHolder(Identifier typeId, IconType<T> type, T content) {
