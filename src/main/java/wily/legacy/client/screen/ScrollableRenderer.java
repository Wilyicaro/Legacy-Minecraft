...
public class ScrollableRenderer {
    ...
    public void render(GuiGraphics graphics, int x, int y, int width, int height, Runnable scrollable) {
        ...
        if (scrolled.max > 0) {
            if (scrolled.get() < scrolled.max)
                scrollRenderer.renderScroll(graphics, ScreenDirection.DOWN, x + width - 13, y + 3 + height);
            if (scrolled.get() > 0)
                scrollRenderer.renderScroll(graphics, ScreenDirection.UP, x + width - 29, y + 3 + height);
        }
    }
