...
public class LegacyLoomScreen extends RecipesScreen<LegacyCraftingMenu, RecipeIconHolder<LegacyLoomScreen.BannerRecipe>> implements TabList.Access {
    ...
    public void resetElements(boolean reposition) {
        ...
        recipeButtons.get(selectedRecipeButton).invalidateFocused();
    ...
    protected void renderLabels(GuiGraphics guiGraphics, int i, int j) {
        ...
        LegacyFontUtil.applySDFont(ignored -> guiGraphics.drawString(this.font, title,((!isSelectionTab() ? imageWidth : imageWidth / 2) - font.width(title)) / 2,accessor.getInteger("title.y", 17), CommonColor.INVENTORY_GRAY_TEXT.get(), false));
        ...
        Component inventoryTitle = menu.inventoryActive ? this.playerInventoryTitle : LegacyComponents.PREVIEW;
        LegacyFontUtil.applySDFont(ignored -> guiGraphics.drawString(this.font, inventoryTitle, inventoryPanelX + (inventoryPanelWidth - font.width(inventoryTitle))/ 2, bottomPanelY + accessor.getInteger("inventoryTitle.y", 11), CommonColor.INVENTORY_GRAY_TEXT.get(), false));
    ...
    protected void updateRecipes() {
        ...
        if (!isSelectionTab() && !selectedStack.isEmpty()) {
            ...
            if (getRecipeButtons().size() > selectedRecipeButton && getRecipeButtons().get(selectedRecipeButton) instanceof LegacyCraftingScreen.CustomCraftingIconHolder h) h.updateRecipe();
        }
    }
    ...
    protected void init() {
        ...
        craftingTabList.setSelected(selectedStack.isEmpty() ? 0 : Math.max(craftingTabList.getIndex(),1));
        menu.inventoryActive = accessor.putBearer("isInventoryActive", Bearer.of(selectedStack.isEmpty()));
        if (selectedRecipeButton < getRecipeButtons().size()) setFocused(getRecipeButtons().get(selectedRecipeButton));
        ...
        if (!isSelectionTab()) {
            recipeButtonsOffset.max = Math.max(0, loomTabListings.get(page.get() * getMaxTabCount() + getTabList().getIndex()).patterns().size() - 12);
            ...
            getRecipeButtons().forEach(b->{
                ...
                b.offset = new Vec3(getRecipeButtons().indexOf(b) * 0.5,0,0);
                b.init();
                addWidget(b);
            });
        }
        ...
        getTabList().init(leftPos, topPos - 37, imageWidth, 43, (t, i) -> {
            ...
            t.offset = (t1) -> new Vec3((LegacyRenderUtil.hasHorizontalArtifacts() && index % 2 != 0 ? 0.0125f : 0.0f) + accessor.getFloat("tabList.buttonOffset.x", -1.5f) * index, t1.active ? t1.selected ? 0 : accessor.getFloat("tabList.selectedOffset.y", 4.4f) : accessor.getFloat("tabList.inactiveOffset.y", 26.4f), 0);
        });
    ...
    protected RecipeIconHolder<BannerRecipe> createRecipeButton(int index) {
        RecipeIconHolder<BannerRecipe> h = new RecipeIconHolder<>(leftPos + 13 + index * 27, topPos + 38) {
            ...
            public boolean keyPressed(int i, int j, int k) {
                ...
                if (controlCyclicNavigation(i, index, recipeButtons, recipeButtonsOffset, scrollRenderer, LegacyLoomScreen.this))
                    return true;
                return super.keyPressed(i, j, k);
            }
            ...
            public void onPress() {
                if (isFocused() && isValidIndex()) {
                    if (LegacyLoomScreen.this.canCraft()) {
                        ...
                        selectedPatterns.forEach(b -> CommonNetwork.sendToServer(new ServerMenuCraftPayload(Optional.of(b.get().pattern.location()), b.getOptionalIngredients(), -1, hasShiftDown() || ControllerBinding.LEFT_STICK_BUTTON.state().pressed)));
        ...
        h.offset = new Vec3(0.5f, 0.5f, 0.0f);
        return h;
    }
    ...
    protected void renderBg(GuiGraphics guiGraphics, float f, int i, int j) {
        ...
        if (!menu.inventoryActive && !previewStack.isEmpty()){
            ...
            guiGraphics.pose().translate(leftPos + inventoryPanelX + inventoryPanelWidth / 2f, topPos + bottomPanelY + bottomPanelHeight / 2f + 10,0);
        ...
        if (craftingTabList.getIndex() == 0) FactoryGuiGraphics.of(guiGraphics).blitSprite(LegacySprites.SQUARE_RECESSED_PANEL, leftPos + descriptionPanelX, topPos + descriptionPanelY, descriptionPanelWidth, descriptionPanelHeight);
        int slotSize = accessor.getInteger("craftingGridSlot.size", 23);
        int xDiff = leftPos + craftingGridPanelX;
        FactoryGuiGraphics.of(guiGraphics).blitSprite(SMALL_ARROW, xDiff + accessor.getInteger("craftingArrow.x", 77) + slotSize / 2, topPos + bottomPanelY + accessor.getInteger("craftingArrow.y", 57), 16, 14);
    ...
    protected boolean controlPage(boolean left, boolean right){
        if ((left || right) && page.max > 0 && craftingTabList.getIndex() != 0){
    ...
    public List<? extends LegacyIconHolder> getRecipeButtons() {
        return isSelectionTab() ? selectBannerButton : recipeButtons;
    }
    ...
    public void render(GuiGraphics guiGraphics, int i, int j, float f) {
        ...
        int resultX = xDiff + accessor.getInteger("resultSlot.x", 104) + slotSize / 2;
        int resultY = yDiff + accessor.getInteger("resultSlot.y", 48);
        for (int index = 0; index < ingredientsGrid.size(); index++)
            LegacyRenderUtil.iconHolderRenderer.itemHolder(xDiff + craftingGridSlotX + index % 3 * slotSize, yDiff + craftingGridSlotY + index / 3 * slotSize, slotSize, slotSize, getActualItem(ingredientsGrid.get(index)), !getActualItem(ingredientsGrid.get(index)).isEmpty() &&  warningSlots[index], new Vec3(LegacyRenderUtil.hasHorizontalArtifacts() ? 0.4f : 0.5f, 0.4f, 0)).render(guiGraphics, i, j, f);
        LegacyRenderUtil.iconHolderRenderer.itemHolder(resultX, resultY, resultSlotSize, resultSlotSize, resultStack, getFocused() instanceof RecipeIconHolder<?> r && r.isValidIndex() && !r.canCraft(), new Vec3(LegacyRenderUtil.hasHorizontalArtifacts() ? 0.4f : 0.5f, 0, 0)).render(guiGraphics, i, j, f);
        if (!resultStack.isEmpty()) {
            ...
            if (LegacyRenderUtil.isMouseOver(i,j,resultX,resultY,resultSlotSize,resultSlotSize)) guiGraphics.renderTooltip(font, resultStack,i,j);
        }
        for (int index = 0; index < ingredientsGrid.size(); index++) if (!getActualItem(ingredientsGrid.get(index)).isEmpty() && LegacyRenderUtil.isMouseOver(i,j,xDiff + craftingGridSlotX + index % 3 * slotSize, yDiff + craftingGridSlotY + index / 3 * slotSize,slotSize,slotSize)) guiGraphics.renderTooltip(font,getActualItem(ingredientsGrid.get(index)),i,j);
