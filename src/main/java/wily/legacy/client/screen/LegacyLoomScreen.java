...
public class LegacyLoomScreen extends RecipesScreen<LegacyCraftingMenu, RecipeIconHolder<LegacyLoomScreen.BannerRecipe>> implements TabList.Access {
    ...
    protected void updateRecipes() {
        ...
        if (!isSelectionTab() && !selectedStack.isEmpty()) {
            ...
            loomTabListings.get(craftingTabList.getIndex()).patterns().stream().filter(p -> Minecraft.getInstance().getConnection().registryAccess().lookupOrThrow(Registries.BANNER_PATTERN).listElementIds().anyMatch(p::equals)).forEach(p -> recipesByGroup.add(Arrays.stream(DyeColor.values()).map(color -> {
                ...
                return RecipeInfo.create(p./*? if <1.21.11 {*//*location*//*?} else {*/identifier/*?}*/().withPrefix(color.getName() + "_"), new BannerRecipe(previewIngs, LegacyCraftingMenu.updateShapedIngredients(new ArrayList<>(Collections.nCopies(9, Optional.empty())), displayIngs, 3, 2, 2), p, color), ings, result);
            ...
            public void onPress(InputWithModifiers input) {
                if (isFocused() && isValidIndex()) {
                    if (LegacyLoomScreen.this.canCraft()) {
                        ...
                        selectedPatterns.forEach(b -> CommonNetwork.sendToServer(new ServerMenuCraftPayload(Optional.of(b.get().pattern./*? if <1.21.11 {*//*location*//*?} else {*/identifier/*?}*/()), b.getOptionalIngredients(), -1, input.hasShiftDown() || ControllerBinding.LEFT_STICK_BUTTON.state().pressed)));
    ...
    protected void renderBg(GuiGraphics guiGraphics, float f, int i, int j) {
        ...
        FactoryGuiGraphics.of(guiGraphics).blitSprite(accessor./*? if <1.21.11 {*//*getResourceLocation*//*?} else {*/getIdentifier/*?}*/("imageSprite", LegacySprites.SMALL_PANEL), leftPos, topPos, imageWidth, imageHeight);
