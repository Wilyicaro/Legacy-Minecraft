...
public class LegacyLoomScreen extends RecipesScreen<LegacyCraftingMenu, RecipeIconHolder<LegacyLoomScreen.BannerRecipe>> implements TabList.Access {
    ...
    protected void extractLabels(GuiGraphicsExtractor GuiGraphicsExtractor, int i, int j) {
        LegacyFontUtil.applySDFont(b -> {
            ...
            GuiGraphicsExtractor.text(this.font, title, ((!isSelectionTab() ? imageWidth : imageWidth / 2) - font.width(title)) / 2, accessor.getInteger("title.y", 17), CommonColor.HTML_COLOR_BLACK.get(), false);
            ...
            if (menu.inventoryActive)
                ...
                GuiGraphicsExtractor.text(this.font, LegacyComponents.PREVIEW, inventoryPanelX + (inventoryPanelWidth - font.width(LegacyComponents.PREVIEW)) / 2, bottomPanelY + accessor.getInteger("inventoryTitle.y", 11), CommonColor.HTML_COLOR_BLACK.get(), false);
        });
    }
    ...
    protected void updateRecipes() {
        ...
        if (!isSelectionTab() && !selectedStack.isEmpty()) {
            ...
            loomTabListings.get(craftingTabList.getIndex()).patterns().stream().filter(p -> Minecraft.getInstance().getConnection().registryAccess().lookupOrThrow(Registries.BANNER_PATTERN).listElementIds().anyMatch(p::equals)).forEach(p -> recipesByGroup.add(Arrays.stream(DyeColor.values()).map(color -> {
                ...
                Optional<Ingredient> dye = Optional.of(Ingredient.of(LegacyItemUtil.getDyeItem(color)));
            ...
            if (getRecipeButtons().size() > selectedRecipeButton && getRecipeButtons().get(selectedRecipeButton) instanceof LegacyCraftingScreen.CustomCraftingIconHolder h)
                h.updateRecipe();
        }
    }
    ...
    protected void init() {
        ...
        int count = accessor.putStaticElement("craftingButtons.count", getRecipeButtons().size());
    ...
    protected RecipeIconHolder<BannerRecipe> createRecipeButton(int index) {
        RecipeIconHolder<BannerRecipe> h = new RecipeIconHolder<>(leftPos + 13 + index * 27, topPos + 38) {
            ...
            public boolean keyPressed(KeyEvent keyEvent) {
                ...
                if (controlCyclicNavigation(keyEvent.key(), index, recipeButtons, recipeButtonsOffset, scrollRenderer, LegacyLoomScreen.this))
                    return true;
                return super.keyPressed(keyEvent);
            }
    ...
    protected void renderBg(GuiGraphicsExtractor GuiGraphicsExtractor, float f, int i, int j) {
        ...
        if (!menu.inventoryActive && !previewStack.isEmpty()) {
            ...
            GuiGraphicsExtractor.pose().translate(leftPos + inventoryPanelX + inventoryPanelWidth / 2f, topPos + bottomPanelY + bottomPanelHeight / 2f + 10);
            float previewScale = accessor.getFloat("preview.scale", 4.25f);
            GuiGraphicsExtractor.pose().scale(previewScale, previewScale);
            GuiGraphicsExtractor.item(previewStack, -8, -8);
        ...
        FactoryGuiGraphics.of(GuiGraphicsExtractor).blitSprite(LegacySprites.SMALL_ARROW, xDiff + accessor.getInteger("craftingArrow.x", 77) + (slotSize / 2), topPos + bottomPanelY + accessor.getInteger("craftingArrow.y", 57), 16, 14);
        if (getTabList().getIndex() != 0) {
