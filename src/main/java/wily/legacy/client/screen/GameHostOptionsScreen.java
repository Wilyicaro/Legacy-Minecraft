    ...
    public GameHostOptionsScreen(Screen parent, Minecraft minecraft) {
        ...
        if (!isOp) {
            for (GameRule<Boolean> key : PlayerInfoSync.All.NON_OP_GAMERULES)
                getRenderableVList().addRenderable(new TickBox(0, 0, Legacy4JClient.gameRules.get(key), b1 -> Component.translatable(key.getDescriptionId()), b1 -> null, b1 -> nonOpGamerules.put(key.id(), b1.selected)));
        ...
        for (GameRule<Boolean> key : WORLD_RULES)
            getRenderableVList().addRenderable(new TickBox(0, 0, Legacy4JClient.gameRules.get(key), b1 -> Component.translatable(key.getDescriptionId()), b1 -> null, b1 -> commandsOnClose.put(b1, () -> minecraft.player.connection.sendCommand("gamerule %s %s".formatted(key.id(), b1.selected)))));
