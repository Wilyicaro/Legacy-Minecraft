...
public abstract class RecipesScreen<T extends AbstractContainerMenu, H extends LegacyIconHolder> extends AbstractContainerScreen<T> implements Controller.Event, ControlTooltip.Event {
    ...
    protected void renderLabels(GuiGraphics guiGraphics, int i, int j) {
        ...
        renderRecipeButtons(guiGraphics, i, j, 0);
        guiGraphics.pose().translate(leftPos, topPos, 0);
    }

    protected void renderRecipeButtons(GuiGraphics guiGraphics, int i, int j, float f) {
        getRecipeButtons().forEach(b -> b.render(guiGraphics, i, j, f));
        if (selectedRecipeButton < getRecipeButtons().size())
            getRecipeButtons().get(selectedRecipeButton).renderSelection(guiGraphics, i, j, f);
    }
    ...
    *///?}
    ...
    public boolean mouseScrolled(double d, double e/*? if >1.20.1 {*/, double f/*?}*/, double g) {
        if (this.getChildAt(d, e).filter((guiEventListener) -> guiEventListener.mouseScrolled(d, e/*? if >1.20.1 {*/, f/*?}*/, g)).isPresent())
            return true;
