...
public interface LegacyNameTag {
    ...
    static void renderNameTagOutline(Font font, MultiBufferSource.BufferSource bufferSource, SubmitNodeStorage.NameTagSubmit submit, boolean seeThrough) {
        ...
        renderOutline(bufferSource.getBuffer(seeThrough ? /*? if <1.21.11 {*//*RenderType*//*?} else {*/RenderTypes/*?}*/.textBackgroundSeeThrough() : /*? if <1.21.11 {*//*RenderType*//*?} else {*/RenderTypes/*?}*/.textBackground()), submit.pose(), submit.x() - 1.1f, submit.y() - 1.1f, font.width(submit.text()) + 2.1f, 10.1f, thickness, color[0], color[1], color[2], 1.0f);
    }
