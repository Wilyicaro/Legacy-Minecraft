...
public final class LegacyMobCaps {
    ...
    public static void handleEntityAdded(Entity entity) {
        if (!(entity.level() instanceof ServerLevel level) || !isEnabled(level)) return;
    ...
    private static void refreshTrackers(MinecraftServer server) {
        ...
        while (iterator.hasNext()) {
            ...
            if (level == null || !isEnabled(level)) iterator.remove();
        }

        for (ServerLevel level : server.getAllLevels()) {
            if (isEnabled(level)) TRACKERS.put(level.dimension(), WorldMobCapTracker.scan(level));
        }
    }
}
