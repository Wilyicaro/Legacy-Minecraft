...
public final class BoxModelManager {
    ...
    public static EnumMap<ToolSlot, float[]> getToolOffsets(ResourceLocation id) {
        return getValue(id, BoxData::toolOffsets);
    }
    ...
    public static EnumMap<AttachSlot, float[]> getAnimationScales(ResourceLocation id) {
        return getValue(id, BoxData::animationScales);
    }

    public static EnumMap<AttachSlot, float[]> getAnimationOffsets(ResourceLocation id) {
        return getValue(id, BoxData::animationOffsets);
    }

    public static EnumMap<AttachSlot, PivotAnimation> getPivotAnimations(ResourceLocation id) {
    ...
    public static EnumMap<AttachSlot, float[]> getArmorScales(ResourceLocation id) {
        return getValue(id, BoxData::armorScales);
    }

    public static Boolean getArmorStretch(ResourceLocation id) {
    ...
    private static ResourceLocation readModelId(String base, ResourceLocation jsonId, boolean skinpacksMode) {
        ...
        if (skinpacksMode) {
            ...
            return wily.factoryapi.FactoryAPI.createLocation(jsonId.getNamespace(), name.substring(0, name.length() - 5));
        }
        String prefix = base + "/";
        if (!path.startsWith(prefix)) return null;
        return wily.factoryapi.FactoryAPI.createLocation(jsonId.getNamespace(), path.substring(prefix.length(), path.length() - 5));
    }
    ...
    private static ResourceLocation resolveModelId(String id) {
        ...
        try {
            return wily.factoryapi.FactoryAPI.createLocation(id);
        } catch (RuntimeException ignored) {
            return null;
        }
    }
    ...
    private static ResourceLocation readTexture(JsonObject texture) {
        ...
        try {
            return wily.factoryapi.FactoryAPI.createLocation(path);
        } catch (RuntimeException ignored) {
            return null;
        }
    }
