...
public class ChangeSkinScreen extends AbstractChangeSkinScreen {
    ...
    private static final ResourceLocation
            SKIN_PANEL = wily.factoryapi.FactoryAPI.createLocation(SkinSync.ASSET_NS, "tiles/skin_panel"),
            PANEL_FILLER = wily.factoryapi.FactoryAPI.createLocation(SkinSync.ASSET_NS, "tiles/panel_filler"),
            PACK_NAME_BOX = wily.factoryapi.FactoryAPI.createLocation(SkinSync.ASSET_NS, "tiles/pack_name_box"),
            SKIN_BOX = wily.factoryapi.FactoryAPI.createLocation(SkinSync.ASSET_NS, "tiles/skin_box"),
            SIZEABLE_ICON_HOLDER = wily.factoryapi.FactoryAPI.createLocation("legacy", "container/sizeable_icon_holder"),
            BEACON_CHECK = wily.factoryapi.FactoryAPI.createLocation("legacy", "textures/gui/sprites/container/beacon_check.png"),
            PADLOCK = wily.factoryapi.FactoryAPI.createLocation("legacy", "textures/gui/sprites/container/padlock.png"),
            HEART_CONTAINER = wily.factoryapi.FactoryAPI.createLocation("minecraft", "hud/heart/container"),
            HEART_FULL = wily.factoryapi.FactoryAPI.createLocation("minecraft", "hud/heart/full");
    ...
    protected Panel createTooltipBox() {
        return new Panel(UIAccessor.of(this)) {
            ...
            public void init(String name) {
                ...
                panel.x = Mth.clamp(panel.x, minX, maxX);
                ...
                panel.y = Mth.clamp(panel.y, minY, maxY);
    ...
    public void renderableVListInit() {
        ...
        addRenderableOnly((g, i, j, f) -> {
            ...
            pose.scale(scale, scaleY, 1.0f);
            pose.translate(-d[0] / 2f, -d[1] / 2f, 0);
            FactoryGuiGraphics.of(g).blit(icon, 0, 0, 0, 0, d[0], d[1], d[0], d[1]);
            pose.popPose();
        });
    ...
    public boolean mouseClicked(double mx, double my, int button) {
        if (handleCarouselMouseClicked(mx, my, button)) return true;
        if (isReorderingCustomPack() && button == InputConstants.MOUSE_BUTTON_LEFT) {
    ...
    public void renderDefaultBackground(GuiGraphics g, int mouseX, int mouseY, float pt) {
        ...
        drawTranslucentSprite(g, SKIN_PANEL, tooltipBox.x - sc(normalLayout.skinPanelInsetX()) - compactPreviewInset, panel.y + sc(normalLayout.skinPanelTop()), Math.max(1, tooltipBox.getWidth() + compactPreviewInset), Math.max(1, tooltipBox.getHeight() - sc(2)));
        drawTranslucentSprite(g, PANEL_FILLER, tooltipBox.x - sc(normalLayout.panelFillerInsetX()), panel.y + sc(normalLayout.panelFillerTop()) + tooltipBox.getHeight() - sc(normalLayout.panelFillerBottomTrim()), Math.max(1, tooltipBox.getWidth() - sc(normalLayout.panelFillerWidthTrim())), Math.max(1, sc(normalLayout.panelFillerHeight())));
        ...
        drawPackNameBox(g, packNameX, packNameY, packNameW, packNameRenderH);
        drawTranslucentSprite(g, SKIN_BOX, tooltipBox.x - sc(normalLayout.skinBoxInsetX()) - compactPreviewInset, panel.y + sc(normalLayout.skinBoxTop()), Math.max(1, tooltipBox.getWidth() - sc(normalLayout.skinBoxWidthTrim()) + compactPreviewInset), Math.max(1, tooltipBox.getHeight() - sc(normalLayout.skinBoxBottomTrim())));
        int holder = Math.max(1, sc(normalLayout.actionHolderSize()));
        int iconX = tooltipBox.x + tooltipBox.getWidth() - sc(normalLayout.actionHolderXOffset());
        ...
        if (center != null) {
            ...
            if (!editing && !isImport && isSkinFavorite(selected)) {
                ...
                float heartScale = heartSize / HEART_TEXTURE_SIZE;
                float centerX = iconX + holder / 2.0f + holder * HEART_HOLDER_OFFSET_X;
                float centerY = iconY + holder / 2.0f + holder * HEART_HOLDER_OFFSET_Y;
                float heartX = centerX - heartSize / 2.0f;
                float heartY = centerY - heartSize / 2.0f;
                g.pose().pushPose();
                g.pose().translate(heartX, heartY, 0);
                g.pose().scale(heartScale, heartScale, 1.0f);
        ...
        if (center != null) {
            ...
            drawScaledCentered(g, Component.literal(PlayerSkinWidget.clipText(minecraft.font, name, maxNameWidth)), mid, skinNameY, ScreenUtil.getDefaultTextColor(true), mainTextScale, true);
            ...
            if (theme != null && !theme.isBlank() && !theme.equals(name)) {
                ...
                drawScaledCentered(g, Component.literal(show), mid, Math.min(themeY, panel.y + tooltipBox.getHeight() - sc(normalLayout.themeBottomInset())), ScreenUtil.getDefaultTextColor(true), mainTextScale, true);
            }
        }
        ...
        if (pack != null) {
            ...
            drawScaledCentered(g, Component.literal(packName), packMid, packTitleY, ScreenUtil.getDefaultTextColor(true), mainTextScale, true);
            Component packLabel = packLabel(pack);
            if (packLabel != null)
                drawScaledCentered(g, packLabel, packMid, packMetaY, ColorUtil.withAlpha(ScreenUtil.getDefaultTextColor(true), 0.8f), packTypeTextScale(), true);
        }
    }

    private void drawPackNameBox(GuiGraphics g, int x, int y, int width, int height) {
        //? if <1.21.3 {
        ScreenUtil.secureTranslucentRender(g, true, 1.0f, ignored -> {
            blitSprite(g, PACK_NAME_BOX, x, y, width, height);
    ...
    private void drawTranslucentSprite(GuiGraphics g, ResourceLocation sprite, int x, int y, int width, int height) {
        //? if <1.21.3 {
        ScreenUtil.secureTranslucentRender(g, true, 1.0f, ignored -> blitSprite(g, sprite, x, y, width, height));
        //?} else {
    ...
    private void drawTick(GuiGraphics guiGraphics, int iconX, int iconY, int holder) {
        ...
        guiGraphics.pose().scale(SKIN_TICK_W / (float) BEACON_CHECK_VISIBLE_W, SKIN_TICK_H / (float) BEACON_CHECK_VISIBLE_H, 1.0f);
        FactoryGuiGraphics.of(guiGraphics).blit(BEACON_CHECK, 0, 0,
                BEACON_CHECK_VISIBLE_X, BEACON_CHECK_VISIBLE_Y,
    ...
    private void drawPadlock(GuiGraphics guiGraphics, int iconX, int iconY, int holder) {
        ...
        FactoryGuiGraphics.of(guiGraphics).blit(PADLOCK, 0, 0, 0, 0, PADLOCK_TEXTURE_SIZE, PADLOCK_TEXTURE_SIZE, PADLOCK_TEXTURE_SIZE, PADLOCK_TEXTURE_SIZE);
        guiGraphics.pose().popPose();
    }
