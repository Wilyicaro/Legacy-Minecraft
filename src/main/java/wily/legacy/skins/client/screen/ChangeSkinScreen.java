...
public class ChangeSkinScreen extends AbstractChangeSkinScreen {
    ...
    private static final ResourceLocation
            CHANGE_SKIN_FRAME = wily.factoryapi.FactoryAPI.createLocation(SkinSync.ASSET_NS, "tiles/change_skin_frame"),
            SIZEABLE_ICON_HOLDER = wily.factoryapi.FactoryAPI.createLocation("legacy", "container/sizeable_icon_holder"),
            PADLOCK_TEXTURE = wily.factoryapi.FactoryAPI.createLocation("legacy", "textures/gui/sprites/container/padlock.png"),
    ...
    private float compactTextScale() {
        return Mth.clamp(uiScale / COMPACT_TEXT_REFERENCE_UI_SCALE, COMPACT_TEXT_MIN_SCALE, 1.0f);
    }
    ...
    private void renderFrameContents(GuiGraphics g, Bounds frame) {
    ...
    private void renderFrameFooter(GuiGraphics g, FrameFooterLayout footer) {
        ...
        blitSprite(g, SIZEABLE_ICON_HOLDER, detailsPanel);
        ...
        blitSprite(g, SIZEABLE_ICON_HOLDER, footer.primaryAction());
        blitSprite(g, SIZEABLE_ICON_HOLDER, footer.secondaryAction());
    }
    ...
    private void drawTick(GuiGraphics guiGraphics, Bounds holder) {
        float guiScale = (float) Math.max(1.0, minecraft.getWindow().getGuiScale());
        int holderFramebufferSize = Math.max(1, Math.round(Math.min(holder.width(), holder.height()) * guiScale));
    ...
    private void drawPadlock(GuiGraphics guiGraphics, Bounds holder) {
        ...
        FactoryGuiGraphics.of(guiGraphics).blit(PADLOCK_TEXTURE, 0, 0, 0, 0,
                PADLOCK_TEXTURE_SIZE, PADLOCK_TEXTURE_SIZE, PADLOCK_TEXTURE_SIZE, PADLOCK_TEXTURE_SIZE);
        guiGraphics.pose().popPose();
    }
    ...
    private void drawFavoriteHeart(GuiGraphics guiGraphics, Bounds holder) {
        ...
        guiGraphics.pose().pushPose();
        guiGraphics.pose().translate(heartX, heartY, 0);
        guiGraphics.pose().scale(heartScaleX, heartScaleY, 1.0f);
