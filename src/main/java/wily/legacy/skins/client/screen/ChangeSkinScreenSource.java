...
public interface ChangeSkinScreenSource {
    ...
    final class Default implements ChangeSkinScreenSource {
        ...
        public boolean renderPreview(GuiGraphics graphics, String skinId, float yawOffset, boolean crouchPose, float attackTime, float partialTick, int left, int top, int right, int bottom) {
            ...
            if (SkinIdUtil.isAutoSelect(skinId)) {
                GuiSessionSkin.SessionSkin sessionSkin = GuiSessionSkin.getSessionSkin();
                if (sessionSkin != null && sessionSkin.texture() != null) {
                    GuiDollRender.renderDollInRect(graphics, skinId, sessionSkin.texture(), sessionSkin.slim(), yawOffset, crouchPose, attackTime, partialTick, left, top, right, bottom, 165);
                    return true;
                }
                SkinEntry fallback = skin(skinId);
                if (fallback == null || fallback.texture() == null) return false;
                GuiDollRender.renderDollInRect(graphics, skinId, fallback.texture(), yawOffset, crouchPose, attackTime, partialTick, left, top, right, bottom, 165);
                return true;
            }
