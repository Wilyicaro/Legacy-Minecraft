...
public class TU3ChangeSkinScreen extends AbstractChangeSkinScreen {
    ...
    private static final ResourceLocation TU3_TOP_STRIP = wily.factoryapi.FactoryAPI.createLocation(SkinSync.ASSET_NS, "tiles/tu3_top_strip"),
            TU3_BOTTOM_STRIP = wily.factoryapi.FactoryAPI.createLocation(SkinSync.ASSET_NS, "tiles/tu3_bottom_strip"),
            TU3_TAB_PLATE = wily.factoryapi.FactoryAPI.createLocation(SkinSync.ASSET_NS, "tiles/tu3_tab_plate"),
            TU3_SELECTED_BADGE = wily.factoryapi.FactoryAPI.createLocation("legacy", "tiles/tu3_selected");
    ...
    private void tintBlitSprite(GuiGraphics g, ResourceLocation sprite, int x, int y, int w, int h, float tint) {
        FactoryGuiGraphics.of(g).setColor(tint, tint, tint, 1.0f);
        blitSprite(g, sprite, x, y, Math.max(1, w), Math.max(1, h));
        FactoryGuiGraphics.of(g).setColor(1.0f, 1.0f, 1.0f, 1.0f);
    }
    ...
    public boolean mouseClicked(double mx, double my, int button) {
        ...
        if (handleCarouselMouseClicked(mx, my, button)) {
            setTu3NavZone(Tu3NavZone.CAROUSEL);
            return true;
        }
