...
public final class SkinSync {
    ...
    public static final int ASSET_BOX_TEXTURE = 4;
    ...
    private static void cacheChunk(UUID owner, String skinId, int assetType, int index, int total, byte[] data) {
        ...
        if ((assetType == ASSET_TEXTURE || assetType == ASSET_BOX_TEXTURE) && bytes.length == 0) {
    ...
    public static void sendSnapshotTo(ServerPlayer requester, MinecraftServer server) {
        ...
        for (Map.Entry<UUID, String> entry : snapshot().entrySet()) {
            sendSnapshotEntry(requester, server, entry.getKey(), entry.getValue());
        }
    }
    ...
    private static void sendSnapshotEntry(ServerPlayer requester, MinecraftServer server, UUID who, String skinId) {
        ...
        if (skinId != null && !skinId.isBlank() && !hasServerAssets(who, skinId)) {
            requestSkin(server.getPlayerList().getPlayer(who));
        }
    }

    @FunctionalInterface
    interface ChunkConsumer {
        void accept(int index, int total, byte[] chunk);
    }
    ...
    public record UploadAssetChunkC2S(String skinId, int assetType, int index, int total,
    ...
    static final class SkinChunkAccumulator {
        ...
        SkinChunkAccumulator(int total) {
