...
public final class StatueOfLibertyPose {
    ...
    public static boolean shouldApply(Object state) {
        if (!ArmPoseSupport.hasPose(state, SkinPoseRegistry.PoseTag.STATUE_OF_LIBERTY)) return false;
        if (ArmPoseSupport.getAttackTime(state) > 0.0F) return false;
    ...
    public static void apply(PlayerModel model, Object state) {
        ...
        if (idleScale > 0.0F) {
            float t = ArmPoseSupport.getIdleSwayTime(state);
