...
public final class SkiingPose {
    ...
    public static boolean shouldApply(AvatarRenderState state) {
        ...
        if (!SkinPoseRegistry.hasPose(SkinPoseRegistry.PoseTag.SKIING, id)) return false;
        return state.id == GuiDollRender.MENU_DOLL_ID || a.consoleskins$isMoving();
    }

    public static void apply(PlayerModel model, AvatarRenderState state, float t) {
        if (model == null) return;
        float speed = 18.0F;
        if (state != null && state.id != GuiDollRender.MENU_DOLL_ID && state instanceof RenderStateSkinIdAccess a) {
            float s = a.consoleskins$getMoveSpeedSq();
            if (s > 0.020F) speed = 34.0F;
            else if (s > 0.004F) speed = 26.0F;
        }
        speed *= 0.1F;
        float swing = (float) Math.sin(t * speed) * 0.35F;
        float base = -0.32F;
