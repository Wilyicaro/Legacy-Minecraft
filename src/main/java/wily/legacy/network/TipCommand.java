...
public class TipCommand {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> commandDispatcher, CommandBuildContext commandBuildContext, Commands.CommandSelection environment) {
        commandDispatcher.register(Commands.literal("legacyTip").requires(commandSourceStack -> commandSourceStack./*? if <1.21.11 {*//*hasPermission(2)*//*?} else {*/permissions().hasPermission(new Permission.HasCommandLevel(PermissionLevel.GAMEMASTERS))/*?}*/).
    ...
    public record EntityPayload(EntityType<?> entityType, boolean force) implements CommonNetwork.Payload {
        ...
        public EntityPayload(CommonNetwork.PlayBuf buf) {
            this(FactoryAPIPlatform.getRegistryValue(buf.get()./*? if <1.21.11 {*//*readResourceLocation*//*?} else {*/readIdentifier/*?}*/(), BuiltInRegistries.ENTITY_TYPE), buf.get().readBoolean());
        }

        @Override
        public void encode(CommonNetwork.PlayBuf buf) {
            buf.get()./*? if <1.21.11 {*//*writeResourceLocation*//*?} else {*/writeIdentifier/*?}*/(BuiltInRegistries.ENTITY_TYPE.getKey(entityType));
            buf.get().writeBoolean(force);
        }
