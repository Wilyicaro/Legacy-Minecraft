...
public class KeyboardControllerInputMixin extends ClientInput {
    ...
    private float calculateForwardImpulse(float original) {
        BindingState.Axis leftStick = ControllerManager.getInstance().getButtonState(ControllerBinding.LEFT_STICK);
        return leftStick.pressed && (keyPresses.forward() || keyPresses.backward()) ? LegacyControlsOptions.smoothMovement.get() && (LegacyControlsOptions.forceSmoothMovement.get() || Legacy4JClient.hasModOnServer()) ? -leftStick.getSmoothY() : (leftStick.y > 0 ? -1 : 1) : original;
    }
    ...
    private float calculateLeftImpulse(float original) {
        BindingState.Axis leftStick = ControllerManager.getInstance().getButtonState(ControllerBinding.LEFT_STICK);
        return leftStick.pressed && (keyPresses.left() || keyPresses.right()) ? LegacyControlsOptions.smoothMovement.get() && (LegacyControlsOptions.forceSmoothMovement.get() || Legacy4JClient.hasModOnServer()) ? -leftStick.getSmoothX() : (leftStick.x > 0 ? -1 : 1) : original;
    }
    ...
    private Vec2 normalize(Vec2 instance, Operation<Vec2> original) {
        return ControllerManager.getInstance().isControllerTheLastInput() && (LegacyControlsOptions.forceSmoothMovement.get() || Legacy4JClient.hasModOnServer()) ? instance : original.call(instance);
    }
}
