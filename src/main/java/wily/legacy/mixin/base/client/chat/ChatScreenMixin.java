...
public abstract class ChatScreenMixin extends Screen implements Controller.Event, ControlTooltip.Event {
    ...
    private void init(CallbackInfo ci){
        ...
        //?} else {
        /*this.input = new EditBox(minecraft.font, 4 + Math.round(LegacyRenderUtil.getChatSafeZone()),height - input.getHeight() + (int)(LegacyRenderUtil.getHUDDistance() - 56), width - (8 + Math.round(LegacyRenderUtil.getChatSafeZone()) * 2), 20, input.getMessage());
        *///?}
    }
    ...
    private void render(CommandSuggestions instance, GuiGraphics guiGraphics, int i, int j){
        ...
        guiGraphics.pose().translate(0,(int)(LegacyRenderUtil.getHUDDistance() - 56),200);
