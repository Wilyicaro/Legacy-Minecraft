...
public class KeyboardControllerInputMixin extends /*? if <1.21.2 {*/Input/*?} else {*//*ClientInput*//*?}*/ {
    ...
    private float calculateForwardImpulse(float original) {
        ...
        return leftStick.pressed && (/*? if <1.21.2 {*/up || down/*?} else {*//*keyPresses.forward() || keyPresses.backward()*//*?}*/ ) ? LegacyOptions.smoothMovement.get() && (LegacyOptions.forceSmoothMovement.get() || FactoryAPIClient.hasModOnServer) ? -leftStick.getSmoothY() : (leftStick.y > 0 ? -1 : 1) : original;
    }
    ...
    private float calculateLeftImpulse(float original) {
        BindingState.Axis leftStick = Legacy4JClient.controllerManager.getButtonState(ControllerBinding.LEFT_STICK);
        return leftStick.pressed && (/*? if <1.21.2 {*/left || right/*?} else {*//*keyPresses.left() || keyPresses.right()*//*?}*/) ? LegacyOptions.smoothMovement.get() && (LegacyOptions.forceSmoothMovement.get() || FactoryAPIClient.hasModOnServer) ?  -leftStick.getSmoothX() : (leftStick.x > 0 ? -1 : 1) : original;
    }
    //? if >=1.21.5 {
