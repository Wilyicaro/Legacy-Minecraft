...
public final class ChangeSkinActions {
    ...
    private static final long FAV_MAX_WAIT_MS = 1200L;
    ...
    private void applySkinIds(List<String> ids, int index, boolean animate) {
        ...
        int startX = x + width / 2 - BASE_W / 2;
        int startY = y + height / 2 - BASE_H / 2;
        ...
        int minStartX = panelLeft + (OFFSET * 4) - 80;

        int rightCardW = (int) (BASE_W * 0.4f) + 6;
        ...
        if (minStartX <= maxStartX) {
            ...
            startX = panelLeft + (available / 2) - (BASE_W / 2);
        }
    ...
    private void warmOne(String id) {
        ...
        if (SkinIdUtil.isCpm(id)) {
            ResourceLocation skinTex = tex != null ? tex : ResourceLocation.fromNamespaceAndPath("skinsync", "skinpacks/default/skins/steve.png");
            try {
