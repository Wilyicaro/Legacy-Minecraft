...
// Build a stable 0..1 coordinate across each cloud face so the fragment shader can soften quad edges.
vec2 getFaceUv(int direction, vec3 faceVertex) {
    if (direction <= 1) {
        return faceVertex.xz;
    } else if (direction <= 3) {
        return faceVertex.xy;
    } else {
        return faceVertex.zy;
    }
}

// Preserve vanilla cloud positioning while exposing face coordinates for fog-driven edge blending.
void main() {
    ...
    faceUv = getFaceUv(direction, faceVertex);
}
