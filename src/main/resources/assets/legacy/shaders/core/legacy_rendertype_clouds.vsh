...
const int FLAG_OFFSET_FACE = 1 << 4;
...
void main() {
    ...
    bool offsetFace = (dirAndFlags & FLAG_OFFSET_FACE) != 0;
    ...
    vec3 pos = (faceVertex + vec3(cellX, 0, cellZ)) * CellSize + CloudOffset;
    if (offsetFace) {
        pos -= faceNormals[direction] * (CellSize / 1024.0);
    }
    gl_Position = ProjMat * ModelViewMat * vec4(pos, 1.0);

    vertexDistance = fog_spherical_distance(pos);
    vertexColor = faceColors[direction] * CloudColor;
