...
public class DataComponentIngredient extends Ingredient {
    ...
    public DataComponentIngredient(HolderSet<Item> items, DataComponentPredicate components, boolean strict) {
        ...
        this.strict = strict;
        this.stacks = items.stream().map(i -> new ItemStack(i, 1, components.asPatch())).toArray(ItemStack[]::new);
    }
    ...
    public static final IIngredientSerializer<DataComponentIngredient> SERIALIZER = new IIngredientSerializer<>() {

        @Override
        public MapCodec<? extends DataComponentIngredient> codec() {
            return CODEC;
        }

        @Override
        public void write(RegistryFriendlyByteBuf arg, DataComponentIngredient arg2) {
            STREAM_CODEC.encode(arg,arg2);
        }

        @Override
        public DataComponentIngredient read(RegistryFriendlyByteBuf arg) {
            return STREAM_CODEC.decode(arg);
        }
    };
}
