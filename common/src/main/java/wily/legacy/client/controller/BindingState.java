...
public abstract class BindingState {
    ...
    public ControlTooltip.ComponentIcon getIcon(){
        return ControlType.getActiveControllerType().getIcons().get(component.getMapped().getSerializedName());
    }
    public boolean canClick(){
        return canClick(getDefaultDelay());
    }
    public boolean canClick(int delay){
        return (timePressed == 0 || timePressed >= 5 * delay) && timePressed % delay == 0 && !isBlocked();
    }
    ...
    public boolean onceClick(boolean block){
        boolean onceClick = onceClick(-(justPressed ? 5 : 1) * getDefaultDelay());
        if (block) block();
        return onceClick;
    }
    ...
    public boolean matches(KeyMapping mapping){
        return ((LegacyKeyMapping)mapping).getBinding() == component;
    }
