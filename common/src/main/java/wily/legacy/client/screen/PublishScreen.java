...
public class PublishScreen extends ConfirmationScreen{
    ...
    public void publish(IntegratedServer server){
        if (!publish) return;
        this.minecraft.gui.getChat().addMessage(publishUnloadedServer(minecraft, gameTypeSlider.getObjectValue(), server.getWorldData().getAllowCommands() && ((LegacyClientWorldSettings)server.getWorldData()).trustPlayers(), this.port) ? PublishCommand.getSuccessMessage(this.port) : Component.translatable("commands.publish.failed"));
    }
