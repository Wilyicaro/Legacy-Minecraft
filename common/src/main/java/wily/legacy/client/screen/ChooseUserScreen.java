...
public class ChooseUserScreen extends PanelVListScreen {
    ...
    public void renderableVListInit() {
        addRenderableOnly(((guiGraphics, i, j, f) -> {
            LegacyGuiGraphics.of(guiGraphics).blitSprite(LegacySprites.PANEL_RECESS,panel.getX() + 10, panel.getY() + 13, panel.getWidth() - 20, panel.getHeight() - 26);
    ...
    protected void addAccountButtons(){
        ...
        CreationList.addIconButton(renderableVList, new ResourceLocation(Legacy4J.MOD_ID,"icon/add_user"),ADD_ACCOUNT,b->minecraft.setScreen(accountScreen(ADD_ACCOUNT, this,true,a-> {
        ...
        for (MCAccount account : MCAccount.list) {
            renderableVList.addRenderable(new AbstractButton(0, 0, 230, 30, account.getMSARefreshToken(null).isEmpty() ? Component.translatable("legacy.menu.offline_user",account.getProfile().getName()) : Component.literal(account.getProfile().getName())) {
                ...
                protected void renderWidget(GuiGraphics guiGraphics, int i, int j, float f) {
                    ...
                    PlayerFaceRenderer.draw(guiGraphics, minecraft.getSkinManager().getInsecureSkinLocation(account.getProfile()), getX() + 5, getY() + 5, 20);
                    if (minecraft.options.touchscreen().get().booleanValue() || isHovered()) {
                        guiGraphics.fill(getX() + 5, getY() + 5, getX() + 25, getY() + 25, -1601138544);
                        LegacyGuiGraphics.of(guiGraphics).blitSprite(ScreenUtil.isMouseOver(i,j,getX()+5,getY()+5,20,20) ? SaveRenderableList.JOIN_HIGHLIGHTED : SaveRenderableList.JOIN, getX() + 5, getY() + 5, 20, 20);
                    }
                }
