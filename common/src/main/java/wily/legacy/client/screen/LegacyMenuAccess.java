...
public interface LegacyMenuAccess<T extends AbstractContainerMenu> extends MenuAccess<T>, GuiEventListener {
    default void movePointerToSlotIn(ScreenDirection direction){
        if (getMenu().slots.isEmpty() || Legacy4JClient.controllerHandler.isCursorDisabled || getHoveredSlot() == null) return;
    ...
    default boolean movePointerToSlot(Slot s, boolean allowHovered){
        ...
        Legacy4JClient.controllerHandler.setPointerPos(holder.getMiddleX() * ((double)minecraft.getWindow().getScreenWidth() / minecraft.getWindow().getGuiScaledWidth()), holder.getMiddleY() * ((double)minecraft.getWindow().getScreenHeight() / minecraft.getWindow().getGuiScaledHeight()));
        return true;
    }
