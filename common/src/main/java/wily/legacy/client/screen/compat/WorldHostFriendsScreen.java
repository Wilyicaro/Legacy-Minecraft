        ...
        if (i == InputConstants.KEY_O){
            ...
            minecraft.setScreen(new ConfirmationScreen(this,230,140,FriendsScreen.ADD_FRIEND_TEXT,WORLD_HOST_FRIEND_USERNAME_TEXT, b->{}){
                ...
                public void render(GuiGraphics guiGraphics, int i, int j, float f) {
                    ...
                    guiGraphics.blitSprite(LegacySprites.SQUARE_ENTITY_PANEL,panel.getX() + panel.getWidth() - 47, panel.getY() + 9, 32,32);
                    if (friend != null){
                        PlayerSkin skin = minecraft.getSkinManager().getInsecureSkin(friend);
                        guiGraphics.pose().pushPose();
                        guiGraphics.pose().translate(1.5f,1.5f,0);
                        PlayerFaceRenderer.draw(guiGraphics, skin, panel.getX() + panel.getWidth() - 47, panel.getY() + 9, 29);
    ...
    protected void addFriendButtons(Runnable afterButtonsAdd){
        ...
        Util.backgroundExecutor().execute(() -> {
            for (UUID uuid : WorldHost.CONFIG.getFriends()) {
                ...
                Supplier<PlayerSkin> skin = minecraft.getSkinManager().lookupInsecure(profile);
