...
public class HostOptionsScreen extends PanelVListScreen {
    ...
    public static List<GameProfile> getActualGameProfiles(){
        ...
        if (minecraft.player != null && !minecraft.player.connection.getOnlinePlayers().isEmpty())
            return minecraft.player.connection.getOnlinePlayers().stream().sorted(Comparator.comparingInt((p -> minecraft.player.getUUID().equals(p.getProfile().getId()) ? 0 : ((LegacyPlayerInfo)p).getPosition()))).map(PlayerInfo::getProfile).toList();
    ...
    protected void pressHostOptionsButton(Button b){
        ...
        PanelVListScreen screen = new PanelVListScreen(this,265,minecraft.player.hasPermissions(2) ? 200 : 118, HOST_OPTIONS){
            @Override
            protected void init() {
                panel.init();
            ...
            public void onClose() {
                super.onClose();
                COMMAND_MAP.values().forEach(Runnable::run);
                if (!nonOpGamerules.isEmpty()) CommonNetwork.sendToServer(new PlayerInfoSync.All(nonOpGamerules));
            }
            public boolean isPauseScreen() {
                return false;
            }
        ...
        if (!minecraft.player.hasPermissions(2)){
            for (GameRules.Key<GameRules.BooleanValue> key : PlayerInfoSync.All.NON_OP_GAMERULES)
                screen.renderableVList.addRenderable(new TickBox(0,0,gameRules.getRule(key).get(),b1-> Component.translatable(key.getDescriptionId()),b1-> null, b1-> nonOpGamerules.put(key.getId(),b1.selected)));
            minecraft.setScreen(screen);
            return;
        }
