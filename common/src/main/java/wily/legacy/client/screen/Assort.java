        ...
        try (Stream<Path> s = Files.walk(path).sorted(Comparator.comparingInt(p-> {
            ...
            for (Path p : ((Iterable<Path>) s::iterator)) {
                ...
                try (BufferedReader r = Files.newBufferedReader(p, Charsets.UTF_8)){
                    list.add(CODEC.parse(JsonOps.INSTANCE,GsonHelper.parse(r)).getOrThrow(false,Legacy4J.LOGGER::warn));
                } catch (IOException e) {
    ...
    public static void save(Path path, List<Assort> assorts, Stocker<String> selected){
        ...
        for (Assort assort : assorts) {
            ...
            try (JsonWriter w = new JsonWriter(Files.newBufferedWriter(p, Charsets.UTF_8))){
                ...
                w.setIndent("  ");
                GsonHelper.writeValue(w,CODEC.encodeStart(JsonOps.INSTANCE,assort).getOrThrow(false,Legacy4J.LOGGER::warn),null);
            } catch (IOException e) {
