...
public record LegacyTipOverride(BiPredicate<Item,DataComponentPatch> matchItemOverride, Predicate<Block> matchBlockOverride, Predicate<EntityType<?>> matchEntityOverride, Component tip) {
    ...
    public static final LoadingCache<Pair<Item, DataComponentPatch>,Component> itemOverrideCache = CacheBuilder.newBuilder().build(new CacheLoader<>() {
        @Override
        public Component load(Pair<Item,DataComponentPatch> itemPair) {
    ...
    public static Component getOverride(ItemStack stack){
        return itemOverrideCache.getUnchecked(Pair.of(stack.getItem(),stack.getComponentsPatch()));
    }
