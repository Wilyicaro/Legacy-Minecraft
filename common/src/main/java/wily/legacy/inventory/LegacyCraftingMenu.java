    ...
    public static LegacyCraftingMenu craftingMenu(Inventory inventory, @Nullable MenuType<?> menuType, int i, BlockPos pos, int gridDimension){
        return new LegacyCraftingMenu(inventory,menuType,i,pos) {
            ...
            public ItemStack getResult(Player player, ServerMenuCraftPacket packet) {
                if (packet.craftId().equals(ServerMenuCraftPacket.EMPTY)) return player.level().getRecipeManager().getRecipeFor(RecipeType.CRAFTING,container,player.level()).map(h-> h.value().assemble(container,player.registryAccess())).orElse(ItemStack.EMPTY);
                return player.level().getRecipeManager().byKey(packet.craftId()).map(h-> h.value() instanceof CraftingRecipe rcp ? rcp.assemble(container,player.registryAccess()) : null).orElse(ItemStack.EMPTY);
            }
