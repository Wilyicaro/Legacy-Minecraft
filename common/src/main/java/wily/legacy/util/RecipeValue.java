...
public interface RecipeValue<C extends Container, T extends Recipe<C>> extends Predicate<T> {
    ...
    Map<ResourceLocation, IdOverride> ID_RECIPE_VALUES_OVERRIDES = new HashMap<>(Map.of(TIPPED_ARROW, (type, manager, rcps, filter) -> {
        manager.byKey(TIPPED_ARROW).ifPresent(h->{
            if (h instanceof TippedArrowRecipe r) {
    ...
    static CustomRecipe overrideCustomRecipe(CustomRecipe recipe, NonNullList<Ingredient> ings, ItemStack result){
        return new CustomRecipe(recipe.getId(),recipe.category()) {
            @Override
    ...
    interface AnyMatch<C extends Container, T extends Recipe<C>> extends RecipeValue<C,T>{
        ...
        default void addRecipes(RecipeType<T> type, RecipeManager manager, List<T> rcps, Predicate<T> filter) {
            manager.getAllRecipesFor(type).stream().filter(h->h.getType() == type && filter.test(h) && test(h)).forEach(rcps::add);
        }
    }
    record BlockTag<C extends Container, T extends Recipe<C>>(TagKey<Block> tag) implements AnyMatch<C,T>{
        @Override
        public boolean test(T h) {
            return h.getResultItem(RegistryAccess.EMPTY).getItem() instanceof BlockItem i && i.getBlock().builtInRegistryHolder().is(tag);
        }
    }
    record ItemTag<C extends Container, T extends Recipe<C>>(TagKey<Item> tag) implements AnyMatch<C,T>{
        @Override
        public boolean test(T h) {
            return h.getResultItem(RegistryAccess.EMPTY).is(tag);
        }
    }
    record Id<C extends Container, T extends Recipe<C>>(ResourceLocation id) implements RecipeValue<C,T>{
        ...
        public void addRecipes(RecipeType<T> type, RecipeManager manager, List<T> rcps, Predicate<T> filter) {
            manager.byKey(id).ifPresent(h->{ if (h.getType() == type && filter.test((T) h)) rcps.add((T) h);});
