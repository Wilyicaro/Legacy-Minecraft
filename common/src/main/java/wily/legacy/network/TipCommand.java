...
public class TipCommand {
    ...
    public static class Packet implements CommonNetwork.Packet {
        ...
        public static Packet decode(RegistryFriendlyByteBuf buf){
            return new Packet().title(ComponentSerialization.STREAM_CODEC.decode(buf)).tip(ComponentSerialization.STREAM_CODEC.decode(buf)).itemIcon(ItemStack.OPTIONAL_STREAM_CODEC.decode(buf)).disappearTime(buf.readInt()).force(buf.readBoolean());
        }
        @Override
