...
public abstract class CartographyTableScreenMixin extends AbstractContainerScreen<CartographyTableMenu> {
    ...
    public void renderBg(GuiGraphics guiGraphics, float f, int i, int j) {
        ...
        guiGraphics.blitSprite(LegacySprites.COMBINER_PLUS_SPRITE,leftPos + 14, topPos + 88,13,13);
        ItemStack input2 = menu.getSlot(1).getItem();
        boolean bl = input2.is(Items.MAP);
        boolean bl2 = input2.is(Items.PAPER);
        boolean bl3 = input2.is(Items.GLASS_PANE);
        ItemStack input = menu.getSlot(0).getItem();
        guiGraphics.blitSprite(LegacySprites.ARROW_SPRITE,leftPos + 36, topPos + 87,22,15);
        if (input.is(Items.FILLED_MAP)) {
            ...
            if (mapItemSavedData != null && (mapItemSavedData.locked && (bl2 || bl3) || bl2 && mapItemSavedData.scale >= 4))
                guiGraphics.blitSprite(LegacySprites.ERROR_CROSS_SPRITE, leftPos + 40, topPos + 87, 15, 15);
            guiGraphics.blitSprite(bl ? LegacySprites.CARTOGRAPHY_TABLE_COPY_SPRITE : bl2 ? LegacySprites.CARTOGRAPHY_TABLE_ZOOM_SPRITE : bl3 ? LegacySprites.CARTOGRAPHY_TABLE_LOCKED_SPRITE : LegacySprites.CARTOGRAPHY_TABLE_MAP_SPRITE,leftPos + 70, topPos + 61,66,66);
        }else{
            guiGraphics.blitSprite(LegacySprites.CARTOGRAPHY_TABLE_SPRITE,leftPos + 70, topPos + 61,66,66);
        }
        guiGraphics.blitSprite(LegacySprites.ARROW_SPRITE,leftPos + 139, topPos + 87,22,15);
    }
}
