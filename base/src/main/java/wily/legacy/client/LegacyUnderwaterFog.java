...
public final class LegacyUnderwaterFog {
    ...
    public static Vector4f getExactFogColor(ClientLevel level, Camera camera) {
        long now = Util.getMillis();
        Entity cameraEntity = camera.entity();
        ...
        if (!underwater || lastLevel != level || lastCameraEntity != cameraEntity || !lastCameraPos.equals(cameraPos)) {
            update(level, cameraEntity, cameraPos, cameraEntity.getY(), now);
        }

        return new Vector4f(
                colorCurrentRed / 256.0F,
                colorCurrentGreen / 256.0F,
                colorCurrentBlue / 256.0F,
                1.0F
        );
    }

    public static boolean setupFog(FogData fogData, Entity cameraEntity, ClientLevel level) {
        if (!isEnabled()) {
            reset();
            return false;
        }
        ...
        float clarity = cameraEntity instanceof LegacyLocalPlayer legacyPlayer
                ? legacyPlayer.getLegacyUnderwaterVisionClarity()
                : 1.0F;
        float visibility = 0.4F + 0.6F * clarity;
        float fogEnd = FOG_END_OFFSET + distanceCurrent * visibility;
    ...
    private static void updateDistanceTarget(float target, long now) {
        if (Float.compare(target, distanceTarget) == 0) {
            return;
        }
    ...
    private static void advanceTransitions(long now) {
        ...
        float distanceProgress = transitionProgress(now, distanceTransitionTime);
        distanceCurrent = Mth.lerp(distanceProgress, distanceCurrent, distanceTarget);
    }
