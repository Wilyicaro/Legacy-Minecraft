...
- If the mod causes important bugs, add the mod to "conflicts" in fabric.mod.json, then add it to "The following mods cause bugs when used with Legacy4J:" in README.md. If the mod is listed but is not in "conflicts", please add it.
- If we can't fix the issue, then try reporting the compatibility issue to that mod's issue tracker.
